...
bool load_content() {
	...
	meshes["box"].get_transform().translate(vec3(4.0f, 2.0f, 4.0f));
	...
	meshes["disk"].get_transform().orientation = rotate(quat(), pi<float>() / 2.0f, vec3(1.0f, 0.0f, 0.0f));
	...
	meshes["torus"].get_transform().orientation = rotate(quat(), pi<float>() / 2.0f, vec3(1.0, 0.0, 0.0));
...
bool update(float delta_time) {
	if (glfwGetKey(renderer::get_window(), GLFW_KEY_DOWN))
	{
		auto aspect = static_cast<float>(renderer::get_screen_width()) / static_cast<float>(renderer::get_screen_height());
		lens += 0.025f;
		//near = 1/tan(quarter_pi<float>()*lens);
		cam.set_projection(quarter_pi<float>()*lens, aspect, near, 1000.0f);
		cam.set_position(cam.get_position() + normalize(cam.get_target()) / lens * 0.6f);
	}
