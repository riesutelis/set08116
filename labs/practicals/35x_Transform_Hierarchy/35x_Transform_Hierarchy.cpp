...
bool update(float delta_time) {
  ...
  // rotate Box one on Y axis by delta_time
	meshes[0].get_transform().rotate(eulerAngleY(delta_time));
  // rotate Box Two on Z axis by delta_time
	meshes[1].get_transform().rotate(eulerAngleZ(delta_time));
  // rotate Box Three on Y axis by delta_time
	meshes[2].get_transform().rotate(eulerAngleY(delta_time));
  // *********************************
  // Update the camera
...
bool render() {
  ...
  for (size_t i = 0; i < meshes.size(); i++) {
    ...
    // SET M to be the usual mesh  transform matrix
	  mat4 M = meshes[i].get_transform().get_transform_matrix();
    // *********************************
