...
bool update(float delta_time) {
	...
	double theta_x = delta_y * ratio_height * 0.7f;
	double theta_y = delta_x * ratio_height * 0.7f;

	// Rotate cameras by delta
	// x - delta_y
	// y - delta_x
	// z - 0
	cam.rotate(vec3(-theta_x, -theta_y, 0.0f));

	// Use keyboard to rotate target_mesh - QE rotate on y-axis
	if (glfwGetKey(renderer::get_window(), GLFW_KEY_Q))
		target_mesh.get_transform().rotate(vec3(0.0f, 0.06f, 0.0f));
	...
	if (glfwGetKey(renderer::get_window(), GLFW_KEY_W))
		target_mesh.get_transform().position = target_mesh.get_transform().position + rotate(target_mesh.get_transform().orientation, vec3(0.0f, 0.0f, -0.1f));
	if (glfwGetKey(renderer::get_window(), GLFW_KEY_S))
		target_mesh.get_transform().position = target_mesh.get_transform().position + rotate(target_mesh.get_transform().orientation, vec3(0.0f, 0.0f, 0.1f));
	if (glfwGetKey(renderer::get_window(), GLFW_KEY_A))
		target_mesh.get_transform().position = target_mesh.get_transform().position + rotate(target_mesh.get_transform().orientation, vec3(-0.1f, 0.0f, 0.0f));
